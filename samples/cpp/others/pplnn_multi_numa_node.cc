...
static inline int32_t GetNumaNodeNum(void) {
#if defined(__linux__) && defined(PPLNN_USE_NUMA)
    if (numa_available() < 0) {
        return 1;
    }
...
static bool Profiling(const vector<string>& input_data, Runtime* runtime, double& avg_run_dur) {
    ...
    avg_run_dur = run_dur / run_count;

    LOG(INFO) << "Profiling End";
    return true;
}

void RunOnNumaNode(int32_t numa_node_id, double& avg_run_dur) {
    LOG(INFO) << "running on numa node " << numa_node_id << ".";

    auto prepare_begin_ts = std::chrono::system_clock::now();

    vector<unique_ptr<Engine>> engines;
    if (!RegisterEngines(&engines, numa_node_id)) {
    ...
    auto diff = std::chrono::duration_cast<std::chrono::microseconds>(run_end_ts - run_begin_ts);
    avg_run_dur = (float)diff.count() / 1000;
    LOG(INFO) << "Run() costs: " << avg_run_dur << " ms.";
    ...
    if (g_flag_enable_profiling) {
        if (!Profiling(input_data, runtime.get(), avg_run_dur)) {
...
int main(int argc, char* argv[]) {
    ...
    const int32_t numa_node_num = GetNumaNodeNum();
    LOG(INFO) << numa_node_num << " numa node detected, will start " << numa_node_num << " engine to run.";

    vector<unique_ptr<thread>> sub_threads;
    vector<double> avg_run_dur(numa_node_num, 0);
    for (int32_t numa_node_id = 0; numa_node_id < numa_node_num; numa_node_id++) {
        sub_threads.emplace_back(new thread(RunOnNumaNode, numa_node_id, ref(avg_run_dur[numa_node_id])));
    }
    for (int32_t numa_node_id = 0; numa_node_id < numa_node_num; numa_node_id++) {
        sub_threads[numa_node_id]->join();
    }

    LOG(INFO) << "all engines run finished.";
    for (int32_t numa_node_id = 0; numa_node_id < numa_node_num; numa_node_id++) {
        LOG(INFO) << "numa node id " << numa_node_id << "'s run avg time: " << avg_run_dur[numa_node_id] << " ms.";
    }
