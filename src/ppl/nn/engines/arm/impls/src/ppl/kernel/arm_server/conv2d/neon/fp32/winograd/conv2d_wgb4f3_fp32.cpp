...
static inline size_t conv_wgb4f3_get_input_buffer_size_fp32(
    const int64_t inC,
...
static inline size_t conv_wgb4f3_get_output_buffer_size_fp32(
    const int64_t outC,
    const int64_t tile_l2_size)
{
    /* inner parallel mode */
...
static inline void conv_wgb4f3_prep_input_block_fp32(
...
static inline void conv_wgb4f3_postp_output_block_fp32(
    ...
    const uint32_t fuse_flag)
{
    ...
    switch (num_valid_oh) {
        case 4:
            switch (num_valid_ow) {
                case 4:
                    vst1q_f32(output_block + outW * (3*OCVL()) + (3*OCVL()), vmid[9]); __attribute__ ((fallthrough));
                case 3:
                    vst1q_f32(output_block + outW * (3*OCVL()) + (2*OCVL()), vmid[8]); __attribute__ ((fallthrough));
                case 2:
                    vst1q_f32(output_block + outW * (3*OCVL()) + (1*OCVL()), vmid[7]); __attribute__ ((fallthrough));
                case 1:
                    vst1q_f32(output_block + outW * (3*OCVL())             , vmid[6]); __attribute__ ((fallthrough));
                default:
                    ;
            }
        ...
        case 3:
            switch (num_valid_ow) {
                case 4:
                    vst1q_f32(output_block + outW * (2*OCVL()) + (3*OCVL()), vmid[5]); __attribute__ ((fallthrough));
                case 3:
                    vst1q_f32(output_block + outW * (2*OCVL()) + (2*OCVL()), vmid[4]); __attribute__ ((fallthrough));
                case 2:
                    vst1q_f32(output_block + outW * (2*OCVL()) + (1*OCVL()), vmid[3]); __attribute__ ((fallthrough));
                case 1:
                    vst1q_f32(output_block + outW * (2*OCVL())             , vmid[2]); __attribute__ ((fallthrough));
                default:
                    ;
            }
            __attribute__ ((fallthrough));
        case 2:
            switch (num_valid_ow) {
                case 4:
                    vst1q_f32(output_block + outW * (  OCVL()) + (3*OCVL()), vmid[1]); __attribute__ ((fallthrough));
                case 3:
                    vst1q_f32(output_block + outW * (  OCVL()) + (2*OCVL()), vmid[0]); __attribute__ ((fallthrough));
                case 2:
                    vst1q_f32(output_block + outW * (  OCVL()) + (1*OCVL()), vio[5]); __attribute__ ((fallthrough));
                case 1:
                    vst1q_f32(output_block + outW * (  OCVL())             , vio[4]); __attribute__ ((fallthrough));
                default:
                    ;
            }
            __attribute__ ((fallthrough));
        case 1:
            switch (num_valid_ow) {
                case 4:
                    vst1q_f32(output_block                     + (3*OCVL()), vio[3]); __attribute__ ((fallthrough));
                case 3:
                    vst1q_f32(output_block                     + (2*OCVL()), vio[2]); __attribute__ ((fallthrough));
                case 2:
                    vst1q_f32(output_block                     + (1*OCVL()), vio[1]); __attribute__ ((fallthrough));
                case 1:
                    vst1q_f32(output_block                                 , vio[0]); __attribute__ ((fallthrough));
                default:
                    ;
            }
            __attribute__ ((fallthrough));
...
uint64_t conv2d_wgb4f3_fp32_runtime_executor::cal_temp_buffer_size() {
    ...
    size_t input_buffer_size = conv_wgb4f3_get_input_buffer_size_fp32(
        cp.channels, sp.tile_blk);
    size_t output_buffer_size = conv_wgb4f3_get_output_buffer_size_fp32(
                ...
                for (int64_t ic_l2 = 0; ic_l2 < ic_g_packed; ic_l2 += k_in_channel_section) {
                    ...
                    for (int64_t ic = 0; ic < in_channel_section; ic += ICVL()) {
                        for (int64_t tile_l0 = 0; tile_l0 < wg_blocks; tile_l0++) {
                                ...
                                conv_wgb4f3_prep_input_block_fp32(
                        ...
                        for (int64_t oc_l2 = 0; oc_l2 < oc_g_packed; oc_l2 += k_out_channel_section) {
                            ...
                            for (int64_t oc = 0; oc < out_channel_section; oc += OCVL()) {
                                for (int64_t tile_l0 = 0; tile_l0 < wg_blocks; tile_l0++) {
                                    ...
                                    conv_wgb4f3_postp_output_block_fp32(
...
static size_t conv_wgb4f3_get_converted_filter_size_fp32(
    const int64_t c_in,
    const int64_t c_out,
...
static void conv_wgb4f3_convert_filter_fp32(
    const float *filter,
...
ppl::common::RetCode conv2d_wgb4f3_fp32_offline_manager::pick_best_schedule_param(const ppl::nn::TensorShape &src_shape, double &run_time, bool tune_blocksize) {
    ...
    const int64_t ic_g_pck = CEIL4(channels/param_.group);
    const int64_t oc_g_pck = CEIL4(num_output/param_.group);
    ...
    uint64_t cvt_filter_size = conv_wgb4f3_get_converted_filter_size_fp32(
        channels, num_output, param_.group) *2;
    ...
    // if (tile_l2_est >= 60) {
    //     if (tile_l2_est % 10 < 9) {
    //         tile_l2_est = (tile_l2_est / 10) * 10;
    //     }
    //     candidate_oc_blk_list = {ic_g_pck};
    //     candidate_ic_blk_list = {oc_g_pck};
    //     candidate_tile_blk_list = {tile_l2_est};
    // }
...
ppl::common::RetCode conv2d_wgb4f3_fp32_offline_manager::gen_cvt_weights(const void *filter, const void *bias) {
    ...
    cvt_filter_size_ = conv_wgb4f3_get_converted_filter_size_fp32(
    ...
    conv_wgb4f3_convert_filter_fp32(
        (const float *)filter,
        (float *)cvt_filter_,
        aux_buffer,
        channels, num_output, param_.group, 
