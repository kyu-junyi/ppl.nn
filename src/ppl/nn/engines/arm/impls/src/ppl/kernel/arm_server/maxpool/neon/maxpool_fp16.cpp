...
static void ppl_arm_server_kernel_fp16_pooling_max_general(
    const __fp16 *input,
    __fp16 *output,
    const int64_t inH,
    const int64_t inW,
...
PRAGMA_OMP_PARALLEL()
{
    ...
    const float16x8_t vmin = vdupq_n_f16(-FLT_MAX);
...
static void ppl_arm_server_kernel_fp16_pooling_max_global(
    const __fp16 *input,
    __fp16 *output,
    const int64_t inH,
    const int64_t inW,
...
static void ppl_arm_server_kernel_fp16_pooling_max_f2s2p0(
    const __fp16 *input,
    __fp16 *output,
    const int64_t inH,
    const int64_t inW,
...
PRAGMA_OMP_PARALLEL()
{
    ...
    for (int64_t n = 0; n < num_batch; n++) {
        ...
        for (int64_t c = 0; c < num_channel; c += CVL()) {
            const __fp16 *input_c_base = input +(n * num_channel_ceil8 + c) * inH * inW;
            __fp16 *output_c_base = output + (n * num_channel_ceil8 + c) * outH * outW;
            for (int64_t oh = 0; oh < outH; oh++) {
                const __fp16 *input_h_base = input_c_base + oh * 2 * inW * CVL();
                __fp16 *output_h_base = output_c_base + oh * outW * CVL();

                int64_t ow = 0;
                for (; ow <= outW-4; ow+=4) {
                    const __fp16 *input_base0 = input_h_base + ow * 2 * CVL();
                    const __fp16 *input_base1 = input_h_base + inW * CVL() + ow * 2 * CVL();
                ...
                for (; ow < outW; ow++) {
                    ...
                    vin[2] = vld1q_f16(input_base + inW * CVL()        );
                    vin[3] = vld1q_f16(input_base + inW * CVL() + CVL());
...
static void ppl_arm_server_kernel_fp16_pooling_max_f3s2(
    const __fp16 *input,
    __fp16 *output,
    const int64_t inH,
    const int64_t inW,
...
PRAGMA_OMP_PARALLEL()
{
    ...
    for (int64_t n = 0; n < num_batch; n++) {
        PRAGMA_OMP_FOR_NOWAIT()
        for (int64_t c = 0; c < num_channel; c += CVL()) {
            const __fp16 *input_c_base = input + (n * num_channel_pck + c) * inH * inW;
            __fp16 *output_c_base = output + (n * num_channel_pck + c) * outH * outW;
            for (int64_t oh = 0; oh < outH; oh++) {
                __fp16 *output_h_base = output_c_base + oh * outW * CVL();
                ...
                if (ih_end - ih_start != 3) {
                    for (int64_t ow = 0; ow < outW; ow++) {
                        ...
                        for (int ih = ih_start; ih < ih_end; ih++) {
                            const __fp16 *input_h_base = input_c_base + ih * inW * CVL();
                    ...
                    const __fp16 *input_h_base = input_c_base + ih_start * inW * CVL();
                    ...
                    for (; ow < ow_no_padding_end; ow+=4) {
                        ...
                        const __fp16 *input_base1 = input_base0 + inW * CVL();
                        const __fp16 *input_base2 = input_base0 + inW * CVL() * 2;
...
static void ppl_arm_server_kernel_fp16_pooling_max_f3s1(
    const __fp16 *input,
    __fp16 *output,
    const int64_t inH,
    const int64_t inW,
    ...
    const int32_t global_pooling)
{
    ...
    if (global_pooling) {
        ppl_arm_server_kernel_fp16_pooling_max_global(src, dst, src_h, src_w, src_c, src_n);
    }
    else if (kernel_h == 2 && kernel_w == 2 && stride_h == 2 && stride_w == 2 && pad_h == 0 && pad_w == 0 && dilation_h == 1 && dilation_w) {
        ppl_arm_server_kernel_fp16_pooling_max_f2s2p0(src, dst, src_h, src_w, dst_h, dst_w, src_c, src_n);
    }
    else if (kernel_h == 3 && kernel_w == 3 && stride_h == 2 && stride_w == 2 && dilation_h == 1 && dilation_w == 1) {
        ppl_arm_server_kernel_fp16_pooling_max_f3s2(src, dst, src_h, src_w, dst_h, dst_w, src_c, src_n, pad_h, pad_w);
    }
    // else if (kernel_h == 3 && kernel_w == 3 && stride_h == 1 && stride_w == 1 && dilation_h == 1 && dilation_w == 1) {
    //     ppl_arm_server_kernel_fp16_pooling_max_f3s1(src, dst, src_h, src_w, dst_h, dst_w, src_c, src_n, pad_h, pad_w);
    // }
    else {
        ppl_arm_server_kernel_fp16_pooling_max_general(src, dst, src_h, src_w, dst_h, dst_w, src_c, src_n, kernel_h, kernel_w, stride_h, stride_w, pad_h, pad_w, dilation_h, dilation_w);
    }
