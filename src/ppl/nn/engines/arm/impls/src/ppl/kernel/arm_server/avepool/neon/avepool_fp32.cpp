...
static void ppl_arm_server_kernel_fp32_pooling_ave_exclude_general(
    const float *input,
    float *output,
    const int64_t inH,
    const int64_t inW,
...
static void ppl_arm_server_kernel_fp32_pooling_ave_include_general(
    const float *input,
    float *output,
    const int64_t inH,
    const int64_t inW,
...
PRAGMA_OMP_PARALLEL()
{
    const int64_t num_channel_ceil4 = CEIL4(num_channel);
    const float32x4_t vzero = vdupq_n_f32(0.0f);
    const float kernel_size_recp = 1.0f / ((float)kerH * (float)kerW);
...
static void ppl_arm_server_kernel_fp32_pooling_ave_global(
    const float *input,
    float *output,
    const int64_t inH,
    const int64_t inW,
...
PRAGMA_OMP_PARALLEL()
{
    const int64_t num_channel_ceil4 = CEIL4(num_channel);
    const float32x4_t vzero = vdupq_n_f32(0.0f);
    const float in_size_recp = 1.0f / ((float)inH * (float)inW);
...
ppl::common::RetCode avepool_n4cx_fp32(
    ...
    if (global_pooling) {
        ppl_arm_server_kernel_fp32_pooling_ave_global(src, dst, src_h, src_w, src_c, src_n);
    }
    else if (count_include_pad) {
        ppl_arm_server_kernel_fp32_pooling_ave_include_general(src, dst, src_h, src_w, dst_h, dst_w, src_c, src_n, kernel_h, kernel_w, stride_h, stride_w, pad_h, pad_w, dilation_h, dilation_w);
    }
    else {
        ppl_arm_server_kernel_fp32_pooling_ave_exclude_general(src, dst, src_h, src_w, dst_h, dst_w, src_c, src_n, kernel_h, kernel_w, stride_h, stride_w, pad_h, pad_w, dilation_h, dilation_w);
    }
