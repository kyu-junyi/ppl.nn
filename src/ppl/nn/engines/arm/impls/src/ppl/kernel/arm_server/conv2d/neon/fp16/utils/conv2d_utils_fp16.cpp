...
{
    const int64_t ic_group_align = FLOOR8(ic_group);
    ...
    const uint32_t fuse_flag)
{
    ...
    int64_t global_oc_inner_start = CEIL8(global_oc_start);
    int64_t global_oc_inner_end   = FLOOR8(global_oc_end);
    ...
    PRAGMA_OMP_SINGLE_NOWAIT()
    if (oc_head) {
        const int64_t global_oc_base        = FLOOR8(global_oc_start);
        ...
        for (int64_t idx = 0; idx < hw_out; idx++) {
            for (int64_t lane = 0; lane < oc_head; lane++) {
                __fp16 tmp_result = output_gbuf_g_base[idx * OCBLK() + lane];
                if (fuse_flag & conv_fuse_flag::SUM) {
                    tmp_result += sum_b_base[global_oc_base * hw_out + idx * OCBLK() + global_oc_ofs_in_lane + lane];
                }
                if (fuse_flag & conv_fuse_flag::RELU) {
                    tmp_result = std::max(tmp_result, (__fp16)0.0f);
                }
                if (fuse_flag & conv_fuse_flag::RELU6) {
                    tmp_result = std::min(tmp_result, (__fp16)6.0f);
                }
                output_b_base[global_oc_base * hw_out + idx * OCBLK() + global_oc_ofs_in_lane + lane] = tmp_result;
            }
        }
    }
    ...
    for (int64_t global_oc = global_oc_inner_start; global_oc < global_oc_inner_end; global_oc += OCBLK()) {
        for (int64_t idx = 0; idx < hw_out; idx++) {
            for (int64_t lane = 0; lane < OCBLK(); lane++) {
                ...
                int64_t buffer_idx                                       = FLOOR8(channel_in_group) * hw_out + idx * OCBLK() + channel_in_group % OCBLK();
                output_b_base[global_oc * hw_out + idx * OCBLK() + lane] = output_gbuf_g_base[buffer_idx];
            }
            if (fuse_flag) {
                float16x8_t vout = vld1q_f16(output_b_base + global_oc * hw_out + idx * OCBLK());
                if (fuse_flag & conv_fuse_flag::SUM) {
                    vout = vaddq_f16(vout, vld1q_f16(sum_b_base + global_oc * hw_out + idx * OCBLK()));
                }
                if (fuse_flag & conv_fuse_flag::RELU) {
                    vout = vmaxq_f16(vout, vzero);
                }
                if (fuse_flag & conv_fuse_flag::RELU6) {
                    vout = vminq_f16(vout, vsix);
                }
                vst1q_f16(output_b_base + global_oc * hw_out + idx * OCBLK(), vout);
            }
        }
    }
    ...
    if (oc_tail) {
        ...
        for (int64_t idx = 0; idx < hw_out; idx++) {
            for (int64_t lane = 0; lane < oc_tail; lane++) {
                ...
                int64_t buffer_idx       = FLOOR8(channel_in_group) * hw_out + idx * OCBLK() + channel_in_group % OCBLK();
                __fp16 tmp_result        = output_gbuf_g_base[buffer_idx];
                if (fuse_flag & conv_fuse_flag::SUM) {
                    tmp_result += sum_b_base[global_oc_base * hw_out + idx * OCBLK() + lane];
                }
                if (fuse_flag & conv_fuse_flag::RELU) {
                    tmp_result = std::max(tmp_result, (__fp16)0.0f);
                }
                ...
                output_b_base[global_oc_base * hw_out + idx * OCBLK() + lane] = tmp_result;
            }
        }
    }
}
