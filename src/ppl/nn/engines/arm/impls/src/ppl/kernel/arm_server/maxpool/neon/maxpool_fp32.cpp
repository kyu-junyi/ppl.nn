...
static void ppl_arm_server_kernel_fp32_pooling_max_general(
    const float *input,
    float *output,
    const int64_t inH,
    const int64_t inW,
...
static void ppl_arm_server_kernel_fp32_pooling_max_global(
    const float *input,
    float *output,
    const int64_t inH,
    const int64_t inW,
...
static void ppl_arm_server_kernel_fp32_pooling_max_f2s2p0(
    const float *input,
    float *output,
    const int64_t inH,
    const int64_t inW,
...
static void ppl_arm_server_kernel_fp32_pooling_max_f3s2(
    const float *input,
    float *output,
    const int64_t inH,
    const int64_t inW,
...
static void ppl_arm_server_kernel_fp32_pooling_max_f3s1(
    const float *input,
    float *output,
    const int64_t inH,
    const int64_t inW,
    ...
    const int64_t padW)
{
    (void)ppl_arm_server_kernel_fp32_pooling_max_f3s1;
PRAGMA_OMP_PARALLEL()
{
    const int64_t num_channel_ceil4 = CEIL4(num_channel);
    ...
    const int32_t global_pooling)
{
    ...
    if (global_pooling) {
        ppl_arm_server_kernel_fp32_pooling_max_global(src, dst, src_h, src_w, src_c, src_n);
    }
    else if (kernel_h == 2 && kernel_w == 2 && stride_h == 2 && stride_w == 2 && pad_h == 0 && pad_w == 0 && dilation_h == 1 && dilation_w) {
        ppl_arm_server_kernel_fp32_pooling_max_f2s2p0(src, dst, src_h, src_w, dst_h, dst_w, src_c, src_n);
    }
    else if (kernel_h == 3 && kernel_w == 3 && stride_h == 2 && stride_w == 2 && dilation_h == 1 && dilation_w) {
        ppl_arm_server_kernel_fp32_pooling_max_f3s2(src, dst, src_h, src_w, dst_h, dst_w, src_c, src_n, pad_h, pad_w);
    }
    // else if (kernel_h == 3 && kernel_w == 3 && stride_h == 1 && stride_w == 1 && dilation_h == 1 && dilation_w) {
    //     ppl_arm_server_kernel_fp32_pooling_max_f3s1(src, dst, src_h, src_w, dst_h, dst_w, src_c, src_n, pad_h, pad_w);
    // }
    else {
        ppl_arm_server_kernel_fp32_pooling_max_general(src, dst, src_h, src_w, dst_h, dst_w, src_c, src_n, kernel_h, kernel_w, stride_h, stride_w, pad_h, pad_w, dilation_h, dilation_w);
    }
