    ...
    const int64_t tile_l2_size)
{
    ...
    const size_t input_buffer_size = CEIL128(WGB4F3_NSET() * CEIL8(inC) * num_wg_blocks * sizeof(__fp16)) + LLC_CACHELINE_SIZE();
    ...
    const int64_t tile_l2_size)
{
    /* inner parallel mode */
    const int64_t num_wg_blocks = tile_l2_size;
    const size_t output_buffer_size = CEIL128(WGB4F3_NSET() * CEIL8(outC) * num_wg_blocks * sizeof(__fp16)) + LLC_CACHELINE_SIZE();
    ...
    const bool iw_valid[WGB4F3_IBLK()])
{
    ...
    if (ih_valid[0]) {
        ...
        v[4] = iw_valid[4] ? vld1q_f16(input_base + 4 * ICVL()) : vzeros;
        v[5] = iw_valid[5] ? vld1q_f16(input_base + 5 * ICVL()) : vzeros;
    }
    else {
        v[0] = vzeros;
        v[1] = vzeros;
    ...
    if (ih_valid[2]) {
        ...
        v[7]  = iw_valid[1] ? vld1q_f16(input_base + 1 * ICVL()) : vzeros;
        v[8]  = iw_valid[2] ? vld1q_f16(input_base + 2 * ICVL()) : vzeros;
        v[9]  = iw_valid[3] ? vld1q_f16(input_base + 3 * ICVL()) : vzeros;
        v[10] = iw_valid[4] ? vld1q_f16(input_base + 4 * ICVL()) : vzeros;
        v[11] = iw_valid[5] ? vld1q_f16(input_base + 5 * ICVL()) : vzeros;
    }
    else {
        v[6]  = vzeros;
        v[7]  = vzeros;
    ...
    if (ih_valid[4]) {
        const __fp16 *input_base = input_block + 4 * inW * ICVL();
        v[18] = iw_valid[0] ? vld1q_f16(input_base             ) : vzeros;
        v[19] = iw_valid[1] ? vld1q_f16(input_base + 1 * ICVL()) : vzeros;
        v[20] = iw_valid[2] ? vld1q_f16(input_base + 2 * ICVL()) : vzeros;
        v[21] = iw_valid[3] ? vld1q_f16(input_base + 3 * ICVL()) : vzeros;
        v[22] = iw_valid[4] ? vld1q_f16(input_base + 4 * ICVL()) : vzeros;
        v[23] = iw_valid[5] ? vld1q_f16(input_base + 5 * ICVL()) : vzeros;
    }
    else {
        v[18] = vzeros;
        v[19] = vzeros;
    ...
    if (ih_valid[3]) {
        ...
        v[13] = iw_valid[1] ? vld1q_f16(input_base + 1 * ICVL()) : vzeros;
        v[14] = iw_valid[2] ? vld1q_f16(input_base + 2 * ICVL()) : vzeros;
        v[15] = iw_valid[3] ? vld1q_f16(input_base + 3 * ICVL()) : vzeros;
        v[16] = iw_valid[4] ? vld1q_f16(input_base + 4 * ICVL()) : vzeros;
        v[17] = iw_valid[5] ? vld1q_f16(input_base + 5 * ICVL()) : vzeros;
    }
    else {
        v[12] = vzeros;
        v[13] = vzeros;
    ...
    // D[1][:]
    if (ih_valid[1]) {
        const __fp16 *input_base = input_block + inW * ICVL();
        v[0] = iw_valid[0] ? vld1q_f16(input_base             ) : vzeros;
        v[1] = iw_valid[1] ? vld1q_f16(input_base + 1 * ICVL()) : vzeros;
        v[2] = iw_valid[2] ? vld1q_f16(input_base + 2 * ICVL()) : vzeros;
        v[3] = iw_valid[3] ? vld1q_f16(input_base + 3 * ICVL()) : vzeros;
    ...
    if (ih_valid[5]) {
        const __fp16 *input_base = input_block + 5 * inW * ICVL();
        v[12] = iw_valid[0] ? vld1q_f16(input_base             ) : vzeros;
        v[13] = iw_valid[1] ? vld1q_f16(input_base + 1 * ICVL()) : vzeros;
        v[14] = iw_valid[2] ? vld1q_f16(input_base + 2 * ICVL()) : vzeros;
        v[15] = iw_valid[3] ? vld1q_f16(input_base + 3 * ICVL()) : vzeros;
    ...
    const uint32_t fuse_flag)
{
    ...
    if (fuse_flag & conv_fuse_flag::SUM) {
        if (num_valid_oh > 0) {
            ...
            if (num_valid_ow > 1) vio[1]  = vaddq_f16(vio[1] , vld1q_f16(sum_block + (1*OCVL()) ));
            if (num_valid_ow > 2) vio[2]  = vaddq_f16(vio[2] , vld1q_f16(sum_block + (2*OCVL()) ));
            if (num_valid_ow > 3) vio[3]  = vaddq_f16(vio[3] , vld1q_f16(sum_block + (3*OCVL()) ));
        }
        if (num_valid_oh > 1) {
            __fp16 *sum_ptr = sum_block + outW * OCVL();
            if (num_valid_ow > 0) vio[4]  = vaddq_f16(vio[4] , vld1q_f16(sum_ptr                ));
            if (num_valid_ow > 1) vio[5]  = vaddq_f16(vio[5] , vld1q_f16(sum_ptr   + (1*OCVL()) ));
            if (num_valid_ow > 2) vmid[0] = vaddq_f16(vmid[0], vld1q_f16(sum_ptr   + (2*OCVL()) ));
            if (num_valid_ow > 3) vmid[1] = vaddq_f16(vmid[1], vld1q_f16(sum_ptr   + (3*OCVL()) ));
        }
        if (num_valid_oh > 2) {
            __fp16 *sum_ptr = sum_block + outW * (2*OCVL());
            if (num_valid_ow > 0) vmid[2] = vaddq_f16(vmid[2], vld1q_f16(sum_ptr                ));
            if (num_valid_ow > 1) vmid[3] = vaddq_f16(vmid[3], vld1q_f16(sum_ptr   + (1*OCVL()) ));
            if (num_valid_ow > 2) vmid[4] = vaddq_f16(vmid[4], vld1q_f16(sum_ptr   + (2*OCVL()) ));
            if (num_valid_ow > 3) vmid[5] = vaddq_f16(vmid[5], vld1q_f16(sum_ptr   + (3*OCVL()) ));
        }
        if (num_valid_oh > 3) {
            __fp16 *sum_ptr = sum_block + outW * (3*OCVL());
            if (num_valid_ow > 0) vmid[6] = vaddq_f16(vmid[6], vld1q_f16(sum_ptr                ));
            if (num_valid_ow > 1) vmid[7] = vaddq_f16(vmid[7], vld1q_f16(sum_ptr   + (1*OCVL()) ));
            if (num_valid_ow > 2) vmid[8] = vaddq_f16(vmid[8], vld1q_f16(sum_ptr   + (2*OCVL()) ));
            if (num_valid_ow > 3) vmid[9] = vaddq_f16(vmid[9], vld1q_f16(sum_ptr   + (3*OCVL()) ));
        }
    }
    ...
    switch (num_valid_oh) {
        case 4:
            switch (num_valid_ow) {
                case 4:
                    vst1q_f16(output_block + outW * (3*OCVL()) + (3*OCVL()), vmid[9]); __attribute__ ((fallthrough));
                case 3:
                    vst1q_f16(output_block + outW * (3*OCVL()) + (2*OCVL()), vmid[8]); __attribute__ ((fallthrough));
                case 2:
                    vst1q_f16(output_block + outW * (3*OCVL()) + (1*OCVL()), vmid[7]); __attribute__ ((fallthrough));
                case 1:
                    vst1q_f16(output_block + outW * (3*OCVL())             , vmid[6]); __attribute__ ((fallthrough));
                default:
                    ;
            }
        ...
        case 3:
            switch (num_valid_ow) {
                case 4:
                    vst1q_f16(output_block + outW * (2*OCVL()) + (3*OCVL()), vmid[5]); __attribute__ ((fallthrough));
                case 3:
                    vst1q_f16(output_block + outW * (2*OCVL()) + (2*OCVL()), vmid[4]); __attribute__ ((fallthrough));
                case 2:
                    vst1q_f16(output_block + outW * (2*OCVL()) + (1*OCVL()), vmid[3]); __attribute__ ((fallthrough));
                case 1:
                    vst1q_f16(output_block + outW * (2*OCVL())             , vmid[2]); __attribute__ ((fallthrough));
                default:
                    ;
            }
            __attribute__ ((fallthrough));
        case 2:
            switch (num_valid_ow) {
                case 4:
                    vst1q_f16(output_block + outW * (  OCVL()) + (3*OCVL()), vmid[1]); __attribute__ ((fallthrough));
                case 3:
                    vst1q_f16(output_block + outW * (  OCVL()) + (2*OCVL()), vmid[0]); __attribute__ ((fallthrough));
                case 2:
                    vst1q_f16(output_block + outW * (  OCVL()) + (1*OCVL()), vio[5]); __attribute__ ((fallthrough));
                case 1:
                    vst1q_f16(output_block + outW * (  OCVL())             , vio[4]); __attribute__ ((fallthrough));
                default:
                    ;
            }
            __attribute__ ((fallthrough));
        case 1:
            switch (num_valid_ow) {
                case 4:
                    vst1q_f16(output_block                     + (3*OCVL()), vio[3]); __attribute__ ((fallthrough));
                case 3:
                    vst1q_f16(output_block                     + (2*OCVL()), vio[2]); __attribute__ ((fallthrough));
                case 2:
                    vst1q_f16(output_block                     + (1*OCVL()), vio[1]); __attribute__ ((fallthrough));
                case 1:
                    vst1q_f16(output_block                                 , vio[0]); __attribute__ ((fallthrough));
                default:
                    ;
            }
            __attribute__ ((fallthrough));
                    ...
                    for (int64_t oc_l2 = 0; oc_l2 < oc_g_packed; oc_l2 += k_out_channel_section) {
                        ...
                        for (int64_t set_id = 0; set_id < WGB4F3_NSET(); set_id ++) {
                            for (int64_t oc = 0; oc < out_channel_section; oc += OCVL()) {
                                for (int64_t block = 0; block < wg_blocks; block += N4CX_HGEMM_N_BLOCK0()) {
                                    const int64_t m_l0 = std::min((int64_t)OCVL(), out_channel_section-oc);
                                    const int64_t n_l0 = std::min((int64_t)N4CX_HGEMM_N_BLOCK0(), wg_blocks-block);
    ...
    const int64_t num_group)
{
    const int64_t ic_group = c_in / num_group;
    const int64_t oc_group = c_out / num_group;
    const size_t converted_filter_size = num_group * WGB4F3_NSET() * CEIL8(oc_group) * CEIL8(ic_group) * sizeof(__fp16) + LLC_CACHELINE_SIZE();
