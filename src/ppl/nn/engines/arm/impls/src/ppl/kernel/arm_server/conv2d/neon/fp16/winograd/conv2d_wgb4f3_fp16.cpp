                        ...
                        for (int64_t set_id = 0; set_id < WGB4F3_NSET(); set_id++) {
                            for (int64_t oc = 0; oc < out_channel_section_align_2ocblk; oc += 2 * OCVL()) {
                                for (int64_t block = 0; block < wg_blocks; block += 10) {
                                    ...
                                    n8cx_hgemm_m16nx_kernel_func_table[n_l0 - 1][init_id][fini_id](
...
static void ppl_arm_server_kernel_fp16_n8cx_hgemm_blocking_an_inner_16x8(
    const __fp16 *a,
    __fp16 *converted_a,
    const int64_t lda,
    const int64_t m,
        ...
        for (int64_t set_id = 0; set_id < WGB4F3_NSET(); set_id++) {
            ...
            for (int64_t i = 0; i < oc_group; i += out_ch_section) {
                for (int64_t p = 0; p < ic_group_packed; p += in_ch_section) {
                    ...
                    ppl_arm_server_kernel_fp16_n8cx_hgemm_blocking_an_inner_16x8(
