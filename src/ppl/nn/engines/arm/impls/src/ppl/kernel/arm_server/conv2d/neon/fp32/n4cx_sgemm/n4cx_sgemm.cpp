...
static void ppl_arm_server_kernel_fp32_n4cx_sgemm_blocking_an_inner_4x4(
    const float *a,
    float *converted_a,
    const int64_t lda,
    const int64_t m,
    ...
    const int64_t k_block1)
{
    for (int64_t i = 0; i < m; i += m_block1) {
        for (int64_t p = 0; p < k; p += k_block1) {
            ...
            ppl_arm_server_kernel_fp32_n4cx_sgemm_blocking_an_inner_4x4(
                a + i * lda + p,
...
// #if 0
template<>
void ppl_arm_server_kernel_fp32_n4cx_sgemm_blocking_an_outer<N4cxSgemmBlockingOrd::N_K_M>(
    const float *a,
    float *converted_a,
    const int64_t lda,
    const int64_t m,
