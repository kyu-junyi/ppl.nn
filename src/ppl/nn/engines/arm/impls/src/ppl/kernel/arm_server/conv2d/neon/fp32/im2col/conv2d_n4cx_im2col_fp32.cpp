        ...
        for (int64_t kh = 0; kh < h_flt; kh++) {
            ...
            for (int64_t kw = 0; kw < w_flt; kw++) {
                ...
                for (int ic = 0; ic < ic_group; ic += ICBLK()) {
                    const float *in_c_base = input_g_base + ic * hw_in;
                    float *i2c_buf_ptr = i2c_buf_kw_base + ic * hw_flt * n_block1;
...
uint64_t conv2d_n4cx_im2col_fp32_runtime_executor::cal_temp_buffer_size() {
    ...
    return conv_n4cx_tile_im2col_get_tmp_buffer_size(
        sp.batch_block3, sp.group_block3, sp.hw_block2,
        src_h*src_w, dst_h*dst_w, cp.kernel_h*cp.kernel_w,
        ic_g_pck, oc_g_pck);
}
...
PRAGMA_OMP_PARALLEL()
{
    ...
    const int64_t m_block1 = ker_param_.sgemm_m_block1;
    const int64_t n_block1 = ker_param_.sgemm_n_block1;
    const int64_t k_block1 = ker_param_.sgemm_k_block1;
    ...
    const int64_t num_group)
{
    const int64_t ic_group = c_in / num_group;
    const int64_t oc_group = c_out / num_group;
    const size_t cvt_filter_size = CEIL128(num_group * PACK_CHANNEL(oc_group) * PACK_CHANNEL(ic_group) * h_flt * w_flt * sizeof(float));
...
ppl::common::RetCode conv2d_n4cx_im2col_fp32_offline_manager::gen_cvt_weights(const void *filter, const void *bias) {
    ...
    conv_n4cx_tile_im2col_convert_filter(
        (const float *)filter, (float *)cvt_filter_, num_output, channels, kernel_h, kernel_w, num_group);
