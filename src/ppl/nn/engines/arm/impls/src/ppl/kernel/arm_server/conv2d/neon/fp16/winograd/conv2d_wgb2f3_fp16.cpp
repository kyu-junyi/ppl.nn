...
#define CHANNEL_VECTOR_LENGTH() 8
    ...
    const int64_t tile_l2_size)
{
    ...
    const size_t input_buffer_size = CEIL128(WGB2F3_NSET() * CEIL8(inC) * num_wg_blocks * sizeof(__fp16)) + LLC_CACHELINE_SIZE();
    ...
    const int64_t tile_l2_size)
{
    /* inner parallel mode */
    const int64_t num_wg_blocks = tile_l2_size;
    const size_t output_buffer_size = CEIL128(WGB2F3_NSET() * CEIL8(outC) * num_wg_blocks * sizeof(__fp16)) + LLC_CACHELINE_SIZE();
    ...
    const bool iw_valid[WGB2F3_IBLK()])
{
    ...
    if (ih_valid[0]) {
        ...
        v[1] = iw_valid[1] ? vld1q_f16(input_base + 1 * ICVL()) : vzeros;
        v[2] = iw_valid[2] ? vld1q_f16(input_base + 2 * ICVL()) : vzeros;
        v[3] = iw_valid[3] ? vld1q_f16(input_base + 3 * ICVL()) : vzeros;
    }
    else {
        v[0] = vzeros;
        v[1] = vzeros;
    ...
    // D[2][:]
    if (ih_valid[2]) {
        const __fp16 *input_base = input_block + 2 * inW * ICVL();
        v[4] = iw_valid[0] ? vld1q_f16(input_base             ) : vzeros;
        v[5] = iw_valid[1] ? vld1q_f16(input_base + 1 * ICVL()) : vzeros;
        v[6] = iw_valid[2] ? vld1q_f16(input_base + 2 * ICVL()) : vzeros;
        v[7] = iw_valid[3] ? vld1q_f16(input_base + 3 * ICVL()) : vzeros;
    }
    else {
        v[4] = vzeros;
        v[5] = vzeros;
    ...
    // D[1][:]
    if (ih_valid[1]) {
        const __fp16 *input_base = input_block + 1 * inW * ICVL();
        v[0] = iw_valid[0] ? vld1q_f16(input_base             ) : vzeros;
        v[1] = iw_valid[1] ? vld1q_f16(input_base + 1 * ICVL()) : vzeros;
        v[2] = iw_valid[2] ? vld1q_f16(input_base + 2 * ICVL()) : vzeros;
        v[3] = iw_valid[3] ? vld1q_f16(input_base + 3 * ICVL()) : vzeros;
    ...
    // D[3][:]
    if (ih_valid[3]) {
        const __fp16 *input_base = input_block + 3 * inW * ICVL();
        v[4] = iw_valid[0] ? vld1q_f16(input_base             ) : vzeros;
        v[5] = iw_valid[1] ? vld1q_f16(input_base + 1 * ICVL()) : vzeros;
        v[6] = iw_valid[2] ? vld1q_f16(input_base + 2 * ICVL()) : vzeros;
        v[7] = iw_valid[3] ? vld1q_f16(input_base + 3 * ICVL()) : vzeros;
    ...
    const uint32_t fuse_flag)
{
    ...
    if (num_valid_oh == 2 && num_valid_ow == 2) {
        ...
        if (fuse_flag & conv_fuse_flag::SUM) { // sum
            ...
            vr[1] = vaddq_f16(vr[1], vld1q_f16(sum_block + OCVL()                ));
            vr[4] = vaddq_f16(vr[4], vld1q_f16(sum_block + outW * OCVL()         ));
            vr[5] = vaddq_f16(vr[5], vld1q_f16(sum_block + outW * OCVL() + OCVL()));
        }
        ...
        vst1q_f16(output_block + OCVL(),                 vr[1]);
        vst1q_f16(output_block + outW * OCVL(),          vr[4]);
        vst1q_f16(output_block + outW * OCVL() + OCVL(), vr[5]);
    }
...
PRAGMA_OMP_PARALLEL()
{
    ...
    // Pack to 4:CVL()
    const int64_t ic_packed = CEIL8(c_in);
    const int64_t oc_packed = CEIL8(c_out);
                ...
                for (int64_t oc_l2 = 0; oc_l2 < oc_g_packed; oc_l2 += k_out_channel_section) {
                    ...
                    for (int64_t set_id = 0; set_id < WGB2F3_NSET(); set_id ++) {
                        for (int64_t oc = 0; oc < out_channel_section; oc += OCVL()) {
                            for (int64_t block = 0; block < wg_blocks; block += N8CX_HGEMM_N_BLOCK0()) {
                                const int64_t m_l0 = std::min((int64_t)OCVL(), out_channel_section-oc);
                                const int64_t n_l0 = std::min((int64_t)N8CX_HGEMM_N_BLOCK0(), wg_blocks-block);
                                ...
                                n8cx_hgemm_kernel_func_table[n_l0-1][init_id][fini_id](
                                    ...
                                    raw_out_cl2_base + set_id * k_out_wg_set_offset + oc * wg_blocks + block * OCVL(),
                                    m_l0,
                                    n_l0,
                                    in_channel_section,
                                    oc_g_packed,
    ...
    const int64_t num_group)
{
    const int64_t ic_group = c_in / num_group;
    const int64_t oc_group = c_out / num_group;
    const size_t converted_filter_size = num_group * WGB2F3_NSET() * CEIL8(oc_group) * CEIL8(ic_group) * sizeof(__fp16) + LLC_CACHELINE_SIZE();
    ...
    for (int64_t g = 0; g < num_group; g++) {
        ...
        for (int64_t oc = 0; oc < oc_group; oc++) {
            for (int64_t ic = 0; ic < ic_group; ic += ICVL()) {
                ...
                // B * G[0][:]
                g_l[0] = vld1q_f32(&g_ic_pck[0]);                g_h[0] = vld1q_f32(&g_ic_pck[4]);
                g_l[1] = vld1q_f32(&g_ic_pck[8]);                g_h[1] = vld1q_f32(&g_ic_pck[12]);
                g_l[2] = vld1q_f32(&g_ic_pck[16]);               g_h[2] = vld1q_f32(&g_ic_pck[20]);
  
                bg_l[0] = g_l[0];                                bg_h[0] = g_h[0];
                bg_l[1] = g_l[1];                                bg_h[1] = g_h[1];
                bg_l[2] = g_l[2];                                bg_h[2] = g_h[2];
                bg_l[3] = vmulq_f32(g_l[0], vhalves);            bg_h[3] = vmulq_f32(g_h[0], vhalves);
                bg_l[4] = vmulq_f32(g_l[1], vhalves);            bg_h[4] = vmulq_f32(g_h[1], vhalves);
                bg_l[5] = vmulq_f32(g_l[2], vhalves);            bg_h[5] = vmulq_f32(g_h[2], vhalves);
                bg_l[6] = bg_l[3];                               bg_h[6] = bg_h[3];
                bg_l[7] = bg_l[4];                               bg_h[7] = bg_h[4];
                bg_l[8] = bg_l[5];                               bg_h[8] = bg_h[5];
  
                // B * G[1][:]  
                g_l[0] = vld1q_f32(&g_ic_pck[24]);               g_h[0] = vld1q_f32(&g_ic_pck[28]);
                g_l[1] = vld1q_f32(&g_ic_pck[32]);               g_h[1] = vld1q_f32(&g_ic_pck[36]);
                g_l[2] = vld1q_f32(&g_ic_pck[40]);               g_h[2] = vld1q_f32(&g_ic_pck[44]);

                bg_l[3] = vfmaq_f32(bg_l[3], g_l[0], vhalves);   bg_h[3] = vfmaq_f32(bg_h[3], g_h[0], vhalves);
                bg_l[4] = vfmaq_f32(bg_l[4], g_l[1], vhalves);   bg_h[4] = vfmaq_f32(bg_h[4], g_h[1], vhalves);
                bg_l[5] = vfmaq_f32(bg_l[5], g_l[2], vhalves);   bg_h[5] = vfmaq_f32(bg_h[5], g_h[2], vhalves);
                bg_l[6] = vfmsq_f32(bg_l[6], g_l[0], vhalves);   bg_h[6] = vfmsq_f32(bg_h[6], g_h[0], vhalves);
                bg_l[7] = vfmsq_f32(bg_l[7], g_l[1], vhalves);   bg_h[7] = vfmsq_f32(bg_h[7], g_h[1], vhalves);
                bg_l[8] = vfmsq_f32(bg_l[8], g_l[2], vhalves);   bg_h[8] = vfmsq_f32(bg_h[8], g_h[2], vhalves);

                // B * G[2][:]
                g_l[0] = vld1q_f32(&g_ic_pck[48]);               g_h[0] = vld1q_f32(&g_ic_pck[52]);
                g_l[1] = vld1q_f32(&g_ic_pck[56]);               g_h[1] = vld1q_f32(&g_ic_pck[60]);
                g_l[2] = vld1q_f32(&g_ic_pck[64]);               g_h[2] = vld1q_f32(&g_ic_pck[68]);

                bg_l[3]  = vfmaq_f32(bg_l[3], g_l[0], vhalves);  bg_h[3]  = vfmaq_f32(bg_h[3], g_h[0], vhalves);
                bg_l[4]  = vfmaq_f32(bg_l[4], g_l[1], vhalves);  bg_h[4]  = vfmaq_f32(bg_h[4], g_h[1], vhalves);
                bg_l[5]  = vfmaq_f32(bg_l[5], g_l[2], vhalves);  bg_h[5]  = vfmaq_f32(bg_h[5], g_h[2], vhalves);
                bg_l[6]  = vfmaq_f32(bg_l[6], g_l[0], vhalves);  bg_h[6]  = vfmaq_f32(bg_h[6], g_h[0], vhalves);
                bg_l[7]  = vfmaq_f32(bg_l[7], g_l[1], vhalves);  bg_h[7]  = vfmaq_f32(bg_h[7], g_h[1], vhalves);
                bg_l[8]  = vfmaq_f32(bg_l[8], g_l[2], vhalves);  bg_h[8]  = vfmaq_f32(bg_h[8], g_h[2], vhalves);
                bg_l[9]  = g_l[0];                               bg_h[9]  = g_h[0];
                bg_l[10] = g_l[1];                               bg_h[10] = g_h[1];
                bg_l[11] = g_l[2];                               bg_h[11] = g_h[2];

                // B*G * B_t[0:1][:]
                g_l[0] = vmulq_f32(bg_l[0], vhalves);            g_h[0] = vmulq_f32(bg_h[0], vhalves);
                g_l[1] = vmulq_f32(bg_l[0], vhalves);            g_h[1] = vmulq_f32(bg_h[0], vhalves);
                g_l[2] = vmulq_f32(bg_l[3], vhalves);            g_h[2] = vmulq_f32(bg_h[3], vhalves);
                g_l[3] = vmulq_f32(bg_l[3], vhalves);            g_h[3] = vmulq_f32(bg_h[3], vhalves);
                g_l[0] = vfmaq_f32(g_l[0], bg_l[1], vhalves);    g_h[0] = vfmaq_f32(g_h[0], bg_h[1], vhalves);
                g_l[1] = vfmsq_f32(g_l[1], bg_l[1], vhalves);    g_h[1] = vfmsq_f32(g_h[1], bg_h[1], vhalves);
                g_l[2] = vfmaq_f32(g_l[2], bg_l[4], vhalves);    g_h[2] = vfmaq_f32(g_h[2], bg_h[4], vhalves);
                g_l[3] = vfmsq_f32(g_l[3], bg_l[4], vhalves);    g_h[3] = vfmsq_f32(g_h[3], bg_h[4], vhalves);
                g_l[0] = vfmaq_f32(g_l[0], bg_l[2], vhalves);    g_h[0] = vfmaq_f32(g_h[0], bg_h[2], vhalves);
                g_l[1] = vfmaq_f32(g_l[1], bg_l[2], vhalves);    g_h[1] = vfmaq_f32(g_h[1], bg_h[2], vhalves);
                g_l[2] = vfmaq_f32(g_l[2], bg_l[5], vhalves);    g_h[2] = vfmaq_f32(g_h[2], bg_h[5], vhalves);
                g_l[3] = vfmaq_f32(g_l[3], bg_l[5], vhalves);    g_h[3] = vfmaq_f32(g_h[3], bg_h[5], vhalves);
                ...
                // B*G * B_t[2:3][:]
                g_l[0] = vmulq_f32(bg_l[6], vhalves);            g_h[0] = vmulq_f32(bg_h[6], vhalves);
                g_l[1] = vmulq_f32(bg_l[6], vhalves);            g_h[1] = vmulq_f32(bg_h[6], vhalves);
                g_l[2] = vmulq_f32(bg_l[9], vhalves);            g_h[2] = vmulq_f32(bg_h[9], vhalves);
                g_l[3] = vmulq_f32(bg_l[9], vhalves);            g_h[3] = vmulq_f32(bg_h[9], vhalves);
                g_l[0] = vfmaq_f32(g_l[0], bg_l[7],  vhalves);   g_h[0] = vfmaq_f32(g_h[0], bg_h[7],  vhalves);
                g_l[1] = vfmsq_f32(g_l[1], bg_l[7],  vhalves);   g_h[1] = vfmsq_f32(g_h[1], bg_h[7],  vhalves);
                g_l[2] = vfmaq_f32(g_l[2], bg_l[10], vhalves);   g_h[2] = vfmaq_f32(g_h[2], bg_h[10], vhalves);
                g_l[3] = vfmsq_f32(g_l[3], bg_l[10], vhalves);   g_h[3] = vfmsq_f32(g_h[3], bg_h[10], vhalves);
                g_l[0] = vfmaq_f32(g_l[0], bg_l[8],  vhalves);   g_h[0] = vfmaq_f32(g_h[0], bg_h[8],  vhalves);
                g_l[1] = vfmaq_f32(g_l[1], bg_l[8],  vhalves);   g_h[1] = vfmaq_f32(g_h[1], bg_h[8],  vhalves);
                g_l[2] = vfmaq_f32(g_l[2], bg_l[11], vhalves);   g_h[2] = vfmaq_f32(g_h[2], bg_h[11], vhalves);
                g_l[3] = vfmaq_f32(g_l[3], bg_l[11], vhalves);   g_h[3] = vfmaq_f32(g_h[3], bg_h[11], vhalves);
