...
#include "ppl/kernel/arm_server/log/neon/log.h"
...
ppl::common::RetCode LogKernel::DoExecute(KernelExecContext* ctx) {
    ...
    if (data_type == ppl::common::DATATYPE_FLOAT32){
        if(MayUseISA(ppl::common::ISA_ARMV8)) {
            return ppl::kernel::arm_server::neon::log_fp32(input->GetShape(), input->GetBufferPtr<float>(),
    ...
    }else if (data_type == ppl::common::DATATYPE_FLOAT16)
    {
        if (MayUseISA(ppl::common::ISA_ARMV8))
        {
           return ppl::kernel::arm_server::neon::log_fp16(input->GetShape(), input->GetBufferPtr<__fp16>(),
