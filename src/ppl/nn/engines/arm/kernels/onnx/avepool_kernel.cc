...
ppl::common::RetCode AvePoolKernel::DoExecute(KernelExecContext* ctx) {
    ...
    int32_t dilation_h;
    int32_t dilation_w;
    kernel_h = param_->kernel_shape.size() >= 1 ? param_->kernel_shape[0] : 1;
    kernel_w = param_->kernel_shape.size() >= 2 ? param_->kernel_shape[1] : 1;
    ...
    PPLNN_ARM_DEBUG_TRACE("include_pads: %s\n", ((include_pads) ? "Y" : "N"));
