...
RetCode OptGraph::Init(ir::Graph* graph, RuntimePartitionInfo* info, EngineOptions* options) {
    ...
    if (options_->marked_tensors != "opt" &&
        options_->marked_tensors.compare(0, 4, "opt:") != 0) {
        auto status = opt_rule_manager->ApplyRule(opt_kernel_options, "MarkTensorRule");
        if (status != RC_SUCCESS) {
            LOG(ERROR) << "Run MarkTensor (before optimization) failed: " << GetRetCodeStr(status);
            return status;
        }
    }
    ...
    acquire_tensor_func_ = [this](edgeid_t eid, uint32_t) -> EdgeObject* {
        auto it = this->tensor_impls_.find(eid);
        if (it == this->tensor_impls_.end()) {
            return nullptr;
        }
...
RetCode OptGraph::DoOptimize(const utils::SharedResource& resource, ArmDevice* device) {
    ...
    status = opt_rule_manager->ApplyRules(options, max_opt_level, "MainLayoutOptimize", "");
    if (status != RC_SUCCESS) {
        LOG(ERROR) << "Run MainLayoutOptimize failed: " << GetRetCodeStr(status);
        return status;
    }

    status = opt_rule_manager->ApplyRules(options, max_opt_level, "AfterLayoutOptimize", "");

    if (options.engine_options->marked_tensors == "opt" ||
        options.engine_options->marked_tensors.compare(0, 4, "opt:") == 0) {
        status = opt_rule_manager->ApplyRule(options, "MarkTensorRule");
        if (status != RC_SUCCESS) {
            LOG(ERROR) << "Run MarkTensor (after optimization) failed: " << GetRetCodeStr(status);
            return status;
        }
    }
