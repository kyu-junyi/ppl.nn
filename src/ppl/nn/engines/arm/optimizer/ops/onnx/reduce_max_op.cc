...
#include "ppl/nn/engines/arm/kernels/onnx/reduce_max_kernel.h"
...
ReduceMaxOp::ReduceMaxOp(const ir::Node* node) : ArmOptKernel(node) {
...
RetCode ReduceMaxOp::SelectAlgoDTypeDFormat(const OptKernelOptions options) {
    return ReduceSelectLayout("ReduceMax", options.io_info, options.engine_options->forward_precision, param_->keepdims, param_->axes, common_param_);
}

#ifdef PPLNN_ENABLE_PMX_MODEL

ppl::common::RetCode ReduceMaxOp::SerializeData(const ::ppl::nn::pmx::SerializationContext& ctx, utils::DataStream* ds) const {
...
ppl::common::RetCode ReduceMaxOp::DeserializeData(const ::ppl::nn::pmx::DeserializationContext& ctx, const void* base, uint64_t size) {
