...
MaxPoolOp::MaxPoolOp(const ir::Node* node) : ArmOptKernel(node) {
...
ppl::common::RetCode MaxPoolOp::SerializeData(const ::ppl::nn::pmx::SerializationContext& ctx, utils::DataStream* ds) const {
...
ppl::common::RetCode MaxPoolOp::DeserializeData(const ::ppl::nn::pmx::DeserializationContext& ctx, const void* base, uint64_t size) {
    auto fb_op_param = ppl::nn::pmx::onnx::GetOpParam(base);

    param_ = std::make_shared<ppl::nn::onnx::PoolingParam>();
    param_->global_pooling = (GetNode()->GetType().name == "GlobalMaxPool") ? 1 : 0;
...
KernelImpl* MaxPoolOp::CreateKernelImpl() const {
    return CreateKernelImplWithParam<ppl::nn::arm::MaxPoolKernel>(param_.get());
}
