...
#include "ppl/nn/engines/arm/kernels/onnx/hard_swish_kernel.h"
...
HardSwishOp::HardSwishOp(const ir::Node* node) : ArmOptKernel(node) {
...
RetCode HardSwishOp::SelectAlgoDTypeDFormat(const OptKernelOptions options) {
    ...
    if (!CheckMajorFloat_(common_param_.input_types[0])) {
        LOG(ERROR) << "Unsupported input type for HardSwish Op: " << GetDataTypeStr(common_param_.input_types[0]);
...
ppl::common::RetCode HardSwishOp::SerializeData(const ::ppl::nn::pmx::SerializationContext& ctx, utils::DataStream* ds) const {
...
ppl::common::RetCode HardSwishOp::DeserializeData(const ::ppl::nn::pmx::DeserializationContext& ctx, const void* base, uint64_t size) {
