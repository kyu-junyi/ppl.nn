...
RetCode ReduceMeanOp::SelectAlgoDTypeDFormat(const OptKernelOptions options) {
    return ReduceSelectLayout("ReduceMean", options.io_info, options.engine_options->forward_precision, param_->keepdims, param_->axes, common_param_);
}
