...
RetCode LogOp::SelectAlgoDTypeDFormat(const OptKernelOptions options) {
    ...
    if (!CheckMajorFloat_(common_param_.input_types[0])) {
        LOG(ERROR) << "Unsupported input type for Log Op: " << GetDataTypeStr(common_param_.input_types[0]);
