...
ReduceSumOp::ReduceSumOp(const ir::Node* node) : ArmOptKernel(node) {
...
RetCode ReduceSumOp::SelectAlgoDTypeDFormat(const OptKernelOptions options) {
    return ReduceSelectLayout("ReduceSum", options.io_info, options.engine_options->forward_precision, param_->keepdims, param_->axes, common_param_);
}

#ifdef PPLNN_ENABLE_PMX_MODEL

ppl::common::RetCode ReduceSumOp::SerializeData(const ::ppl::nn::pmx::SerializationContext& ctx, utils::DataStream* ds) const {
...
ppl::common::RetCode ReduceSumOp::DeserializeData(const ::ppl::nn::pmx::DeserializationContext& ctx, const void* base, uint64_t size) {
